// DocumentWalker.cpp
// - span 적용 + covered 스킵 + empty cell 표시
// - 점유표(occupancy grid)는 "테이블 루트 단위 로컬" 유지 (중첩표 안전)
// - CellMode 저장/복원 포함
...
// Table renderer (NEW)
// =====================
static void RenderTableFromRoot373(OWPML::CObject* tableRoot, std::wstring& out, int depth)
{
    if (!tableRoot) return;
    ...
    for (int r = 0; r < rowCount; ++r)
    {
        ...
        for (int c = 0; c < colCount; ++c)
        {
            ...
            {
                // occupied도 아니고 wrapper도 없으면: 구조 유지용 empty td
